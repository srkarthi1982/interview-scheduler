...
export const generateSuggestionsForInterview = (params: {
  meetingDate: string;
  ...
  for (
    let startUtcMs = candidateStart;
    startUtcMs + durationMs <= latestEnd;
    startUtcMs += STEP_MINUTES * MINUTE_MS
  ) {
  ...
  for (const candidate of sorted) {
    ...
    if (deduped.length >= MAX_VISIBLE_SUGGESTIONS) break;
  }

  return deduped.map((candidate, index) => {
    const label: SuggestionLabel =
      index === 0 && candidate.requiredCoverage > 0
        ? "best"
