...
export const PARTICIPANT_TYPE_OPTIONS: ParticipantType[] = [
  "candidate",
  "interviewer",
  "observer",
];
...
export const formatParticipantTypeLabel = (participantType: ParticipantType) =>
  participantType.charAt(0).toUpperCase() + participantType.slice(1);

export const normalizeInterviewRow = (
  row: any,
  ...
  candidateEmail: normalizeText(row.candidateEmail ?? "") || null,
  roleTitle: normalizeText(row.roleTitle),
  companyName: normalizeText(row.companyName ?? "") || null,
  interviewStage: normalizeText(row.interviewStage),
  meetingDate: String(row.meetingDate),
  durationMinutes: Number(row.durationMinutes ?? 0),
  notes: normalizeMultilineText(row.notes ?? ""),
  status: (row.status ?? "draft") as InterviewDTO["status"],
  selectedSuggestionId: row.selectedSuggestionId ? String(row.selectedSuggestionId) : null,
...
export const normalizeParticipantRow = (row: any): ParticipantDTO => ({
  ...
  participantType: (row.participantType ?? "interviewer") as ParticipantType,
...
export const buildDetailResponse = (params: {
  ...
  selectedSuggestion: params.suggestions.find((suggestion) => suggestion.isSelected) ?? null,
});
