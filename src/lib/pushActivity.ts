...
import type { InterviewSchedulerDashboardSummaryV1 } from "../dashboard/summary.schema";
...
type InterviewSchedulerActivity = {
  event: string;
  occurredAt: string;
  entityId?: string;
};

export const pushInterviewSchedulerActivity = async (params: {
  userId: string;
  activity: InterviewSchedulerActivity;
  summary: InterviewSchedulerDashboardSummaryV1;
}): Promise<void> => {
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn("pushInterviewSchedulerActivity failed", error);
    }
  }
};
